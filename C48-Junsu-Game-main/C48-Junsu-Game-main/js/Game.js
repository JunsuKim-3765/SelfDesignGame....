class Game {
  ...
  start() {
    ...
    medicineG = new Group();
      ...
      for(var i=0; i<650;i=i+40){
        medicine = createSprite(1950,40+i,20,20);
        //medicine.x = 70+i
        medicine.addImage("medicine",medicineImg);
        medicine.scale = 0.3;
        medicineG.add(medicine)
      ...
      for(var i=0; i<650;i=i+40){
        medicine = createSprite(300,40+i,20,20);
        //medicine.x = 70+i
        medicine.addImage("medicine",medicineImg);
        medicine.scale = 0.3;
        medicineG.add(medicine)
      ...
      for(var i=0; i<900;i=i+40){
        medicine = createSprite(1380,300+i,20,20);
        //medicine.x = 70+i
        medicine.addImage("medicine",medicineImg);
        medicine.scale = 0.3;
        medicineG.add(medicine)
  ...
  playOne(){
    ...
    if(allPlayers != undefined ){
      ...
      textSize(15)
      text(life,117,60);
      text('medicinecount: '+ medicineCount,117,90);
      ...
      for(var plr in allPlayers){
        ...
        if(player.nation==="KOREA"){
          mynation.addImage("mynation",koreaImg);
          mynation.scale = 0.03;
        }

        if(player.nation==="USA"){
          mynation.addImage("mynation",usaImg);
          mynation.scale = 0.03;
        }

        if(player.nation==="SINGAPORE"){
          mynation.addImage("mynation",singaporeImg);
          mynation.scale = 0.03;
        }

        if(player.nation==="GREAT BRITAIN"){
          mynation.addImage("mynation",ukImg);
          mynation.scale = 0.03;
        }

        if(player.nation==="ARGENTINA"){
          mynation.addImage("mynation",argentinaImg);
          mynation.scale = 0.03;
        }

        if(player.nation==="CANADA"){
          mynation.addImage("mynation",canadaImg);
          mynation.scale = 0.03;
        }

        if(player.nation==="CHINA"){
          mynation.addImage("mynation",chinaImg);
          mynation.scale = 0.03;
        }

        if(player.nation==="RUSSIA"){
          mynation.addImage("mynation",russiaImg);
          mynation.scale = 0.03;
        }

        if(player.nation==="JAPAN"){
          mynation.addImage("mynation",japanImg);
          mynation.scale = 0.03;
        }

        if(player.nation==="ISRAEL"){
          mynation.addImage("mynation",israelImg);
          mynation.scale = 0.03;
        }
       ...
       //image(myname,x,y,10,10);
        ...
        //bullet.isTouching(zombieG,this.destroyZ);
        // if(bullet.isTouching(zombieG)){
        //   this.destroyZ();
        // };
  ...
  handlePlayerControls() {
    ...
    // if(keyDown('space')){
    //   bullet = createSprite(myplayer.position.x,myplayer.position.y-10,10,10);
